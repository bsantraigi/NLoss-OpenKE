...
from config_fix.adjlist import AdjList
...
class Config(object):
    ...
    def sampling(self):
        ...
        # h, t, r
        access = np.random.randint(0, 2, inv_degrees.shape)
        neighbors_h = np.random.randint(0, self.adjList.nE, inv_degrees.shape)*access
        neighbors_t = np.random.randint(0, self.adjList.nE, inv_degrees.shape)*(1-access)

        neighbors = np.concatenate([neighbors, neighbors], 0)
        # print(neighbors.shape)
        # print(inv_degrees.shape)
        neighbors[inv_degrees.shape[0]:, 0] = (neighbors_h + neighbors[inv_degrees.shape[0]:, 0]) % self.adjList.nE
        neighbors[inv_degrees.shape[0]:, 1] = (neighbors_t + neighbors[inv_degrees.shape[0]:, 1]) % self.adjList.nE
    ...
    def train_one_step(self, neighbors, inv_degrees=None):
        ...
        sh = neighbors.shape[0]//2
        self.trainModel.batch_y = to_var(
            np.concatenate(
                [np.ones(sh, dtype=np.float32), -np.ones(sh, dtype=np.float32)]
            ), self.use_gpu)
