...
class Config(object):
    ...
    def save_embedding_matrix(self, best_model):
        path = os.path.join(self.result_dir, self.model.__name__ + "_OLD.json")
        f = open(path, "w")
    ...
    def set_test_model(self, model, path=None):
        ...
        if path == None:
            path = os.path.join(self.result_dir, self.model.__name__ + "_OLD.ckpt")
    ...
    def save_checkpoint(self, model, epoch):
        path = os.path.join(
            self.checkpoint_dir, self.model.__name__ + "_OLD-" + str(epoch) + ".ckpt"
        )
